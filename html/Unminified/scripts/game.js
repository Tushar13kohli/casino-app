...
class Game extends Phaser.Scene {
	...
	create(){
		...
		// this.add.tileSprite(0,config.height,config.width,0,'footer').setOrigin(0,1);
		...
		let txt_line = this.add.text(0, 0, String(game_config.cur_payline), {fontFamily: 'bebas', fontSize: 30, align: 'center',color: '#FFFFFF'}).setOrigin(0.5);
		let txt_bet = this.add.text(0, 0, String(game_config.cur_bet), {fontFamily: 'bebas', fontSize: 30, align: 'center',color: '#FFFFFF'}).setOrigin(0.5);
		let txt_win = this.add.text(0, 0, '0', {fontFamily: 'bebas', fontSize: 30, align: 'right',color: '#FFFFFF'}).setOrigin(1,0.5);
		let txt_total = this.add.text(0, 0, String(total_bet), {fontFamily: 'bebas', fontSize: 30, align: 'right',color: '#FFFFFF'}).setOrigin(1,0.5);
		//End text
		...
		function generate_result() {
			fetch('https://demo3096670.mockable.io/spin', {
			method:'POST'
		})
		.then(response => response.json())
			.then(data => {
				win = data.win;
					...
					for(let i=0; i<5; i++){
						stop_at[i] = parseInt(resultNumber.toString().split('')[i], 10) -1
					}
